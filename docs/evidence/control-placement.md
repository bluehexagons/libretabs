...
`python3 scripts/verify.py` passed with 6,222 core checks, 355 practice UI
...
web persistence. Changing that live document back to bottom exposed and led to
fixing a stale container allocation; the final transition restored the complete
bottom bar inside the viewport. The interaction also found and led to fixing a
missing pair of keys in the web bridge allow-list. The final fresh-document
console contained only the Godot engine, WebGL and build-configuration messages;
no page error or failed network request was reported.
