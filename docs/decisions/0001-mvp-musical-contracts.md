...
| Pause / Resume | Freeze the audible source position and silence voices; Resume continues there without a fresh count-in. Reconstruct notes whose sounding intervals cross the destination, including supported sustain state; do not replay earlier attacks as a burst. The prototype oscillator restores nominal phase and gain for notes crossing a seek or loop boundary; full channel/sustain-state restoration remains a later audio milestone. |
